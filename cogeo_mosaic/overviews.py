...
def create_high_level_cogs(
    ...
    """
    Create High resolution (~COGs) mosaic image from a mosaicJSON.

    The output will be a web optimized COG with bounds matching the mosaicJSON bounds and
    with its resolution matching the mosaic MaxZoom.

    Note: gdalbuiltvrt + rio cogeo create might be a better solution for creating a high
    resolution image from a set of COGs.

    """
    pixel_method = PIXSEL_METHODS[method]
    with MosaicBackend(mosaic_path) as mosaic:
        quadkey_zoom = mosaic.quadkey_zoom
        maxzoom = mosaic.metadata["maxzoom"]
        bounds = mosaic.metadata["bounds"]
        mosaic_quadkeys = set(mosaic._quadkeys)
        ...
        extrema = tile_extrema(bounds, quadkey_zoom)
        resolution = _meters_per_pixel(maxzoom, 0, tilesize=256)

        tilesize = 256 * (2 ** (mosaic.metadata["maxzoom"] - quadkey_zoom))
        blocks = list(_get_blocks(extrema, tilesize=tilesize))
        random.shuffle(blocks)

        width = (extrema["x"]["max"] - extrema["x"]["min"]) * tilesize
        height = (extrema["y"]["max"] - extrema["y"]["min"]) * tilesize
        w, n = mercantile.xy(
            *mercantile.ul(extrema["x"]["min"], extrema["y"]["min"], quadkey_zoom)
        )
        ...
        with rasterio.Env(**config):
            with ExitStack() as ctx:
                ...
                def _get_tile(wind):
                    ...
                    x = extrema["x"]["min"] + idx[1]
                    y = extrema["y"]["min"] + idx[0]
                    t = mercantile.Tile(x, y, quadkey_zoom)
                    assets = mosaic.tile(*t)
                    if not assets:
                        return window, None, None
                ...
                with futures.ThreadPoolExecutor(max_workers=threads) as executor:
                    future_work = [executor.submit(_get_tile, item) for item in blocks]
                    with click.progressbar(
                        futures.as_completed(future_work),
                        length=len(future_work),
                        show_percent=True,
